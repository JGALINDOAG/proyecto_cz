...
class Privilegio extends AccesoDatos
{
    ...
    public function get_id_privilegio($cvePrivilegio)
    {
        try {
            ...
            die("¡Error!: get_cve_privilegio() " . $e->getMessage());
        }
    }
    ...
    public function delete_id_privilegio($cvePrivilegio)
    {
        try {
            $this->dbh = AccesoDatos::conexion();
            // $cvePrivilegio = AccesoDatos::desencriptar(str_replace(' ', '+', $cvePrivilegio));
            $query = "UPDATE privilegio SET activo = 0 WHERE cve_privilegio = ?";
            $stmt = $this->dbh->prepare($query);
            $stmt->bindValue(1, $cvePrivilegio, PDO::PARAM_INT);
            $stmt->execute();
            $this->dbh = null;
        } catch (PDOException $e) {
            die("¡Error!: delete_cve_privilegio() " . $e->getMessage());
        }
    }
    ...
    public function lastIndex()
    {
        try {
            $this->dbh = AccesoDatos::conexion();
            $query = "SELECT MAX(cve_privilegio) AS max FROM privilegio";
            $stmt = $this->dbh->prepare($query);
            if ($stmt->execute()) {
